 ...
 * BriefingBuilderView
 *
 * JSX wrapper that connects the MVP module to the TypeScript Briefing Builder.
 * Converts KYC data from the app context into ValidationContext format.
 */
...
function convertToKYCResponse(kycData, mvpData, tasteProfile) {
  // Map from app's data structure to kyc-schema structure
  return {
    propertyContext: {
...
/**
 * BriefingBuilderView Component
 */
export default function BriefingBuilderView({
  kycData,
  mvpData,
  tasteProfile,
  projectId,
  ...
  // Generate validation context from KYC
  const validationContext = useMemo(() => {
    try {
      return mapKYCToValidation(kycResponse);
    } catch (error) {
      console.error('Error mapping KYC to validation:', error);
      return null;
    }
  }, [kycResponse]);

  // Handle save
  const handleSave = (planBrief) => {
    console.log('Saving PlanBrief:', planBrief);
    if (onSave) {
      onSave(planBrief);
    }
    ...
    return (
      <div className="p-6">
        ...
        <div className="bg-red-50 border border-red-200 rounded-lg p-4">
          <h3 className="text-red-800 font-medium">Unable to load Briefing Builder</h3>
          <p className="text-red-600 text-sm mt-1">
            Please ensure KYC data is complete before proceeding.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="briefing-builder-view">
      <BriefingBuilder
