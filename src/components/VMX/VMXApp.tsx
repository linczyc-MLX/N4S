...
import { useN4SContext } from "./utils/n4s-context";
...
export default function App() {
  ...
  // N4S integration: read context from parent app if hosted
  const n4sContext = useN4SContext();
  const [n4sApplied, setN4sApplied] = useState(false);
  ...
  // N4S Integration: Apply context from parent app on mount
  useEffect(() => {
    if (n4sApplied || !n4sContext.isHosted || !n4sContext.values) return;

    const { values } = n4sContext;
    console.log("[VMX] Applying N4S context:", values);

    // Apply area
    if (values.areaSqft && values.areaSqft > 0) {
      setAreaSqft(values.areaSqft);
    }
    ...
    // Apply region (if it exists in library)
    if (values.regionId && library.regions.some((r) => r.id === values.regionId)) {
      setRegionAId(values.regionId);
    }

    // Apply compare mode
    if (values.compareMode) {
      setCompareMode(true);
      if (values.regionBId && library.regions.some((r) => r.id === values.regionBId)) {
        setRegionBId(values.regionBId);
      }
    }

    setN4sApplied(true);
  }, [n4sContext, n4sApplied, library]);
    ...
    <div className="container">
      <div className="topBar">
        <div>
          ...
          {n4sContext.isHosted && n4sContext.values && (
            <div className="n4sProjectInfo">
              <span className="n4sProjectName">{n4sContext.values.projectName}</span>
              {n4sContext.values.clientName && (
                <span className="n4sClientName">({n4sContext.values.clientName})</span>
              )}
            </div>
          )}
        </div>
